...
### تمدید گواهی: `.well-known` نباید پروکسی شود

هر vhost در `haman-domains.conf` مسیر `/.well-known/` را از پروکسی مستثنا
می‌کند. بدون این، چالش ACME به لاراول می‌رسد و ۴۰۴ می‌گیرد — و گواهی سه ماه
بعد بی‌صدا تمدید نمی‌شود، بدون اینکه در این فاصله چیزی خراب به نظر برسد.

روی پورت ۸۰ این کار با `RewriteCond` انجام شده نه با `Alias` تنها، چون
mod_rewrite قبل از mod_alias اجرا می‌شود.

**هشدار جداگانه:** `.htaccess` مربوط به `api.arshanweb.ir` چنین استثنایی
ندارد و همه‌چیز را پروکسی می‌کند. تمدید گواهی آن دامنه احتمالاً همین حالا
خراب است. دست‌نخورده رها شده تا جدا بررسی شود.
