...
class Haman_Product_Sync {
    ...
    // reads from (Sync tab, haman_field_mapping option) and
    // authenticity_fields() below. Shared with class-haman-admin.php's
     ...
     * haman_field_mapping option: field => {type: 'meta'|'attribute', key})
    ...
    private function authenticity_fields( \WC_Product $p ): array {
        $mapping = get_option( 'haman_field_mapping', [] );
    ...
    private function product_pdf_attachments( \WC_Product $p ): array {
        if ( get_option( 'haman_sync_pdfs', '1' ) !== '1' ) return [];
