 ...
 * (X-Haman-Signature, sha256=hex(hmac_sha256(body, webhook_secret)))
 ...
 * per call.
 */
class Haman_Live_Query_Handler {
    ...
    public function register_routes(): void {
        // Registered under both namespaces, same handlers. The platform and
        // the plugin update independently and in either order, so for a
        // while a 2.0.0 plugin will be called on the 1.x path and a 1.9.0
        // plugin on nothing else. See Haman_Legacy for when this goes.
        foreach ( [ 'haman/v1', Haman_Legacy::REST_NAMESPACE ] as $namespace ) {
            $this->register_in( $namespace );
        }
    }

    private function register_in( string $ns ): void {
        register_rest_route( $ns, '/live-query', [
            'methods'             => 'POST',
            'callback'            => [ $this, 'handle' ],
            // Auth is HMAC-based below, not WP's native REST permission
            // system — same posture as class-haman-webhook-handler.php.
        ...
        register_rest_route( $ns, '/trigger-sync', [
            'methods'             => 'POST',
    ...
    public function handle_trigger_sync( WP_REST_Request $req ): WP_REST_Response {
        ...
        $version = defined( 'HAMAN_VERSION' ) ? HAMAN_VERSION : '';
        ...
        if ( ! class_exists( 'Haman_Sync_Manager' ) ) {
        ...
        set_transient( 'haman_last_manual_sync', $started, DAY_IN_SECONDS );
    ...
    private function check_sync_rate_limit(): bool {
        $last = (int) get_transient( 'haman_manual_sync_guard' );
        if ( $last ) return false;
        set_transient( 'haman_manual_sync_guard', time(), self::SYNC_MIN_INTERVAL_HOURS * HOUR_IN_SECONDS );
        return true;
    }
    ...
    private function verify_signature( WP_REST_Request $req ): bool {
        // A platform that has not been updated yet still signs with the
        // old header name; the signature itself is unchanged.
        $sig = $req->get_header( 'X-Haman-Signature' );
        if ( empty( $sig ) ) {
            $sig = $req->get_header( Haman_Legacy::SIGNATURE_HEADER );
        }
    ...
    /** Same cached-from-server secret Haman_Api_Client::send_webhook()
     * already uses for the outbound direction — never stored as a
     * permanent local option (see class-haman-admin.php's Connection tab
     * docblock), just cached an hour at a time. */
    private function get_secret(): string {
        $cached = get_transient( 'haman_webhook_secret_cache' );
        if ( false !== $cached ) return (string) $cached;
        $secret = ( new Haman_Api_Client() )->get_webhook_secret();
        if ( is_wp_error( $secret ) ) return '';
        set_transient( 'haman_webhook_secret_cache', $secret, HOUR_IN_SECONDS );
        return $secret;
    }

    private function check_rate_limit(): bool {
        $key   = 'haman_toolcall_rl_' . gmdate( 'YmdHi' ); // fixed 1-minute bucket
     ...
     * _payment_link_rule() and haman-widget.js's
    ...
    private function create_draft_order( array $body ): WP_REST_Response {
        ...
        // (cancel_stale_draft_orders(), Haman_Sync_Manager) and the
        ...
        $order->update_meta_data( '_haman_created', 1 );
        $order->update_meta_data( '_haman_chatbot_id', sanitize_text_field( (string) get_option( 'haman_chatbot_id', '' ) ) );
    ...
    private function order_tracking_code( WC_Order $order ): ?string {
        foreach ( [ '_tracking_number', '_wc_shipment_tracking_number', '_haman_tracking',
                    Haman_Legacy::old_key( '_haman_tracking' ), 'tracking_code' ] as $key ) {
