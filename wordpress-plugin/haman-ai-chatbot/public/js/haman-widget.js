/**
 * Haman AI Chatbot — front-end widget.
 *
 * Config arrives via wp_localize_script as window.HamanWidgetConfig (see
 * Haman_Public::build_config() in class-haman-public.php). Everything the
...
(function () {
    ...
    var CFG = window.HamanWidgetConfig;
    if (!CFG || !CFG.chatbotId || !CFG.apiUrl) return;

    var CONV_STORAGE_KEY = 'haman_conv_v1';
    var OPEN_STORAGE_KEY = 'haman_open_v1';
    ...
    function persistConv(sessionId, convId) {
        ...
        // Haman_Sync_Manager::on_order_placed() attach this conversation
        ...
        try {
            document.cookie = 'haman_conv_id=' + encodeURIComponent(convId) +
    ...
    // the chatbot's text direction are independent settings; haman-widget.css
    ...
    // in CFG at page-render time (see class-haman-public.php). Nothing is
    ...
    function openWidget() {
        ...
        // haman-widget.css keys off this class directly on the host so the
