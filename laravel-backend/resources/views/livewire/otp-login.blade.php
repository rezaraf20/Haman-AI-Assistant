@php
    ...
    $primary = '#1B3A6B';
    $inputStyle = "width:100%;box-sizing:border-box;padding:10px 12px;border:1px solid #CBD5E1;border-radius:8px;font-size:14px;background:#fff;color:#0F172A;";
    $labelStyle = "display:block;font-size:13px;margin-bottom:6px;color:#334155;";
    $btnStyle = "width:100%;padding:10px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;background:{$primary};color:#ffffff;";
...
@endphp
<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;" dir="rtl">
    <div style="width:100%;max-width:380px;background:#ffffff;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,.08);padding:28px;">
        <h1 style="font-size:18px;font-weight:700;margin:0 0 20px;text-align:center;color:#0F172A;">ورود / ثبت‌نام</h1>
        ...
        @if ($step === 'phone')
            <form wire:submit="sendCode">
                ...
                <button type="submit" style="{{ $btnStyle }}" wire:loading.attr="disabled">ارسال کد تایید</button>
            </form>
        @elseif ($step === 'otp')
            <form wire:submit="verifyCode">
                <p style="font-size:13px;color:#64748B;margin:0 0 16px;">کد ارسال‌شده به {{ $phone }} را وارد کنید.</p>
                <div style="margin-bottom:16px;">
                ...
                <button type="submit" style="{{ $btnStyle }}" wire:loading.attr="disabled">تایید</button>
                <div style="display:flex;justify-content:space-between;margin-top:14px;font-size:13px;">
                    <button type="button" wire:click="backToPhone" style="background:none;border:none;color:#64748B;cursor:pointer;">تغییر شماره</button>
                    <button type="button" wire:click="resendCode" style="background:none;border:none;color:{{ $primary }};cursor:pointer;">ارسال مجدد کد</button>
                </div>
            </form>
        @elseif ($step === 'profile')
            <form wire:submit="completeProfile">
                <p style="font-size:13px;color:#64748B;margin:0 0 16px;">برای تکمیل ثبت‌نام، اطلاعات زیر را وارد کنید.</p>
                <div style="margin-bottom:16px;">
                ...
                <button type="submit" style="{{ $btnStyle }}" wire:loading.attr="disabled">تکمیل ثبت‌نام و ورود</button>
            </form>
        @endif
    </div>
</div>
